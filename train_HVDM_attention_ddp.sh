...
if [ "$1" == "256" ]; then
CUDA_VISIBLE_DEVICES=5 python3 scripts/evaluation/train_HVDM_attention_ddp.py \
  --seed ${seed} \
  --ckpt_path $ckpt \
  --config $config \
  ...
  --int_dimension 64 \
  --attack all \
  --output_dir model_pth_HVDM_attenton_1003
# --data_path ../video

#accelerate launch
else
CUDA_VISIBLE_DEVICE=5 python3 scripts/evaluation/train_.py \
--seed ${seed} \
--ckpt_path $ckpt \
--config $config \
--savedir $res_dir/$name \
